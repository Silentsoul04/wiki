...
- SQL-инъекция обычно происходит, если разработчик веб-приложения не может гарантировать, что значение, полученное из
  веб-формы, файлов cookie, входных параметров и т. Д., Будет передано в запрос SQL (который выполняется на сервере базы
...
`order by` Поскольку это оператор сортировки, вы можете использовать условный оператор для вынесения суждений и оценки
