---
title: Атаки на публичную экспоненту
---

## Атака с малым индексом открытого ключа

### Условия атаки

$e$ очень мало, например $e$ = 3.

### Принцип атаки

Предположим, пользователь использует ключ $e=3$. Учитывая, что отношение шифрования выполнено:
...
Злоумышленник может перечислить $k$ от мала до велика и по очереди открыть три корня, пока не будет открыто целое число.

### Пример

??? example "XMan Summer Camp"

    Здесь мы возьмем в качестве примера упражнение класса XMan Summer Camp (у Джарвиса OJ есть повторение), 
    у вложения есть `flag.enc` и `pubkey.pem`, которые, очевидно, являются зашифрованным текстом и открытым ключом. 
    Openssl прочтите открытый ключ.
    ...
    -----END PUBLIC KEY-----
    ```
    Видя $e=3$, ясно, что атаковали малый индекс открытого ключа. Здесь мы используем библиотеку Crypto для чтения 
    открытого ключа и используем многопроцессорность для ускорения взлома.
    ...
    Перебор занял достаточно долгое время, получили флаг
    
    ```text
    Didn't you know RSA padding is really important? Now you see a non-padding message is so dangerous. And you should 
    notice this in future. Fl4g: flag{Sm4ll_3xpon3nt_i5_W3ak}
    ```

## Производный алгоритм RSA - алгоритм Рабина

### Условия атаки

Алгоритм Рабина характеризуется $e=2$.
...
- Рассчитайте $m_p$ и $m_q$:

    $$\begin{align*} m_p &= \sqrt{c} \bmod p \\ 
    m_q &= \sqrt {c} \bmod q \end{align*}$$

- Рассчитайте $y_p$ и $y_q$ расширенным алгоритмом Евклидова:

    $$y_p \cdot p + y_q \cdot q = 1$$

- Решите четыре открытых текста:
  
    $$\begin{align*} a &= (y_p \cdot p \cdot m_q + y_q \cdot q \cdot m_p) \bmod n \\ 
    b &= n - a \\ 
...
!!! info 
    ...
    Если $p \equiv q \equiv 3 \pmod 4$, тогда

    $$\begin{align*} m_p &= c^{\frac{1}{4}(p + 1)} \bmod p \\ m_q &= c^{\frac{1}{4}(q + 1)} \bmod q \end{align*}$$

    В целом $p \equiv q \equiv 3 \pmod 4$ достаточно, в случаях, которые не выполняются, обратитесь к соответствующему 
    алгоритму.
...
??? example "XMan Summer Camp"
    ...
    Здесь мы берем упражнение класса XMan Summer Camp (у Джарвиса О.Дж. есть повторение) в качестве примера, 
    ...
    $e=2$, рассмотрим алгоритм Рабина. Сначала сломаем $p$ и $q$.
